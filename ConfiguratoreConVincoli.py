...
def carica_vincoli(path_opzioni):
    """Carica tutti i vincoli dal CSV delle opzioni."""
    vincoli = {}
    with open(path_opzioni, newline='', encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if vincoli_raw:
                try:
                    vincoli[opzione] = json.loads(vincoli_raw)
                except Exception:
                    try:
                        import ast
                        vincoli[opzione] = ast.literal_eval(vincoli_raw)
                    except Exception as e:
                        logging.error(f"Errore parsing vincoli per opzione '{opzione}': {vincoli_raw} - {e}")
    return vincoli

def verifica_vincoli(macchina, stazione, selezioni_correnti, vincoli):
    """
    Restituisce True se la selezione corrente è compatibile con i vincoli.
    macchina: es. 'MCV1'
    stazione: es. '2'
    selezioni_correnti: dict {stazione: {opzione: valore}}
    vincoli: dict caricato da carica_vincoli
    """
    for opzione, vincoli_opz in vincoli.items():
        if macchina in vincoli_opz:
            if stazione in vincoli_opz[macchina]:
                for opz_bloccante, valori_bloccati in vincoli_opz[macchina][stazione].items():
                    for stz, scelte in selezioni_correnti.items():
                        if opz_bloccante in scelte and scelte[opz_bloccante] in valori_bloccati:
                            return False  # C'è un blocco
